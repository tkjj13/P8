...
\va{$Sens$}{is the sensitivity of the receiver}{dB}
...
\va{$Te$}{is the thermal temperature}{Kelvin}
...
This gives us the \gls{SNR} margin needed to distinguish a deep fade from the noise floor. % given a set confidence interval and margin.
...
\end{equation}
Given that we set our confidence interval of $\pm 1$dB around the value of our constant signal in the Rician \gls{CDF} matched with the 90\% confidence level on the probability axis, we are able to estimate numerically the part of the K-factor that we are interested in.
\todo {matlab script in appendix}
\todo{source on this whole?}
\begin{figure}[H]
\centering
