...
\begin{where}
\va{$DR$}{Dynamic range of the system}{dB}
\va{$MAX_{str}$}{is the maximum strength of a receiving system}{dBm}
...
\end{equation}
\begin{where}
\va{$BW$}{is the bandwidth}{Hz}
\va{$Te$}{The thermal temperature of the system}{Kelvin}
\va{$NF$}{Noise figure of the receiver}{dB}
\va{$k$}{Boltzmann's constant}{1}
\va{$SNR_{Req}$}{Required SNR}{dB}
\end{where}



Dynamic Range or else \gls{SINAD} is also very similar to the \gls{SNR}. \gls{SINAD} is a good way to describe the whole dynamic system of \gls{ADC} because it takes into account all the components that make noise and distortion, including the non-linearities. %In other words it takes into account the non-linearities.
...
\end{figure}
To calculate the dynamic range we are interested on the third order products.The second order products they get filtered out due to the big distance from the original frequencies.When we have the third order products we draw a vertical line from the intercept point of the sensitivity and the third order products till the fundamental signal this is the $MAX_{STR}$ of a receiver. This distance between the two lines defines the strength of the strong output signals that will cause distortion at the sensitivity level. In our example above the intercept point of the third order products and the sensitivity level is at $-100dBm$ while the vertical line meets the fundamental signal at the $-32dBm$ meaning that the dynamic range is $68dB$. 
...
\begin{where}
\va{$IF_{BW}$}{intermediate bandwidth}{Hz}
\va{$Te$}{The thermal temperature of the system}{Kelvin}
\va{$k$}{Boltzmann's constant}{1}
\va{$AVG_f$}{is the averaging factor}{1}
\va{$NF$}{Noise figure}{dB}
\end{where}
...
\va{$BW_{sweep}$}{total bandwidth you want to measure}{Hz}
\end{where}
...
The SNR requirement becomes:
\begin{equation}
SNR = SNR_m - G_f , 14dB + 50dB = 64dB 
\end{equation}
\begin{where}
