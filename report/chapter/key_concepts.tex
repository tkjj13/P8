...
\begin{equation}
Sens = 10log(BW\cdot Te\cdot k_b\cdot 1000) + NF + SNR_{Req}
\label{Noise1}
\end{equation}
\begin{where}
\va{Sens}{is the sensitivity of the receiver}{dBm}
...
\begin{align}
\text{SINAD} &= 20log\left(\frac{s}{w+p_d}\right)\\
...
\va{$p_d$}{is the distortion power}{W}
\end{where}
...
\begin{equation}
W = log_{2}(AVG_f)\cdot 3dB +10log(IF_{BW}\cdot T_{e}\cdot k_b\cdot 1000) + NF 
\label{NFwithAVG}
\end{equation}
