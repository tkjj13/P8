...
\va{$k$}{is the Boltzmann's constant}{$\frac{J}{K}$}
...
\text{SNR} &= 20log\left(\frac{s}{w}\right) \label{eq:SNR_formula}
\end{align}

\begin{where}
\va{SINAD}{is the signal to noise and distortion ratio}{1}
\va{SNR}{is the signal to noise ratio}{1}
\va{$s$}{is the signal power}{W}
\va{$w$}{is the noise power}{W}
\va{$d$}{is the distortion power}{W}
\end{where}
...
\caption{A Typical block diagram of a VNA receiver \citep{agilent_VNA}.}
\label{Block_VNA}
\end{figure}
...
\begin{equation}
W = log_{2}(AVG_f)\cdot 3dB +10log(IF_{BW}\cdot T_{e}\cdot k\cdot 1000) + NF 
\label{NFwithAVG}
\end{equation}
...
\va{$BW_{sweep}$}{is the total bandwidth you want to measure}{Hz}
\end{where}
...
\begin{equation}\label{needed_SNR}
...
\begin{equation}
N_{tot} = N_a \cdot \underbrace{\frac{f_{span}}{B_c }}_{N_f} \cdot \underbrace{\frac{A}{(\frac{\lambda}{2})^2}}_{N_d}
\label{howtosample}
\end{equation} 
...
\begin{equation}
A  = \frac{N_{tot}\cdot (\frac{\lambda}{2})^2}{N_a\cdot \frac{f_{span}}{B_c}}
\label{howtosqaure}
\end{equation}
