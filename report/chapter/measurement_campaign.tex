...
\section{Setup}
To find the delay spread and ultimately the coherence bandwidth a \gls{PDP} measurement is necessary. The PNA is preforming a inverse FFT of the frequency to get to the time.
\section{Results}
The PDP 
\section{Data processing}
\subsection{Delay spread}
The PDP profile is measured at 20 different positions in the measuring area in \autoref{meas_area}. These PDPs are averaged across to get a overall PDP of the area. It's clearly a exponential PDP and this means that the coherence bandwidth can be estimated from the previous used approximation from \autoref{}. First the RMS delay spread is calculated from the measured PDP. This gives $\sigma_{\tau} = 14ns$. This is a lower value then the estimated spread.
...
\end{equation}
This approximation method gives a higher $B_C$ then expected for the frequency to be uncorrelated. This would give a number of sweep points $N_{sweep} = 14$ and would increase our measurement time drastically.

\subsection{Coherence bandwidth test}
To figure out a balanced value of our $B_C$  a extra test was done. By doing a measurement with $B_C = 5MHz$ and making \gls{ACF} of the values:
\todo{insert pic of autocorrelation test}
Her the test show a correlation of <0.2 with a $B_C = 25MHz$. This value is alot closer to the estimation. This would give a  $N_{sweep} = 40$.
