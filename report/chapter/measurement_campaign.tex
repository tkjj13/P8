...
\section{FCF}
The delay spread of a signal is very depandant on the enviroemnt. For this reason a pilot test will measure the delay in our pilot test. Simelar indoor values has been measured and shows anything from 20ns delay to 200ns delay for small office eniforments\citep{indoor_delay}. Since the measuremnt what to measure NLOS conditions $\sigma_{\tau}$ of 50ns is assumed. With the RMS delay spread we can find the approximated $B_c$ which is inversely proportional. 
\begin{equation}
...
There is also some overhead for saving the data. Since number of points used in the final setup is dependant $B_c$ and the $\Delta f$. If we use the span of 1GHz and a Coherence bandwidth of 20MHz this gives 50 points. This will reduce the sweep time by quarter.

Let's use example where we can use a 300Hz $IF_BW$ a
\begin{equation}
T_{sweep total} = \frac{631}{4} = 160ms
\end{equation}
This means that every 160ms we have to move $\frac{\lambda}{2}$ this gives a relative velocity of:
\begin{equation}
\frac{0.0375m}{0.160s} = 0.23 m/s
\end{equation}
This is under the $1m/s$ value found in \autoref{min_vel}
and total time for moving 5 meters $\approx 22s$.
...
\end{equation}
or around 21 minuets of measurement given 55.5 walks and 22s per walk. We suspect these values to change depending on the speed at which the data can be saved and the needed $IF_{BW}$.
