...
\end{equation}
So it is only dependant on one parameter, the delay $\tau$. The $|R_x(\tau)| = \mathop{\mathbb{E}}(h(t))^2)$, witch is the average power of $h(t)$. These autocorrelations are found with double Fourier transform and gives a stochastic average power of the channel:
\begin{center}
...
\begin{equation}
S(f_d) = \mathcal{F}(R(\Delta d )) = \int_{- \infty}^{\infty} R(\Delta d)e^{-j2 \pi \Delta d f} d \Delta d
\end{equation}
\begin{equation}
