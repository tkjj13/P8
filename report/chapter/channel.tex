...
By having several antennas at both/or the receiver and transmitter a antenna diversity is achieved. With several antennas the signal can be combined together to process the signals into one. To get independently fading signals the signals need to be uncorrelated by separating the antennas least half a wavelength (micro diversity) \citep{diversityAntenna}. Antenna diversity comes at the cost of a more complex system or increased power consumption.
...
\section{Doppler}
Doppler spread is when the velocity of a wireless system causes a frequency shift for the signal of each path. The BW of a channel must be larger then the Doppler spread to function. Doppler spread is related to the velocity of the system given by:
\begin{equation}
Doppler Spread = 2/\lambda \cdot v
\end{equation}
Where $\lambda$ is wavelength and $v$ is velocity.

If we assume the system is moving slowly at $5m/s$ and we use a open $2.4Ghz$ frequency we can see that the Doppler spread is 80Hz. This means our bandwidth can't be lower than 80Hz. We know that the dynamic range of system is dependant on the IF bandwidth, and that we should use as low BW as possible. A 80Hz bandwidth is very narrow and will give us little frequency dependant fading and we can assume frequency flat fading. Given this definition of Doppler spread we can assume that this should not be a problem since our measurements BW must be higher to get enough samples.
...
\end{equation}
If the units of $\Delta d$ is meters,then $f_d$ is in cycles per meter. The $\Leftrightarrow$ represents the Fourier transform. The space domain can be hard to visualize considering it can have 3 dimensions where $\Delta d = (x,y,z)$. This can be limited to 1 or 2 dimensions for simplification. \citep{FTandCORR}
...
Since the channel behaviour are hard to predict, the normal method is to use statistical models to describe a channel in terms of probabilities. So the channel impulse respond $h(t)$ can be considered as random process.
...
\begin{equation}
R(t_1,t_2)_h = R(\tau) \ , \  \tau =t_2 - t_1  
\end{equation}
So it is only dependant on one parameter, the delay $\tau$. The $|R(\tau)| = \mathop{\mathbb{E}}(h(t))^2)$, which is the average power of $h(t)$. The autocorrelation of $g(\Delta d)$ in the spatial domain gives a relationship that is dependant on the spatial position $\Delta d$. The joint autocorrelations are found with a double Fourier transform and gives a stochastic average power of the channel:
\begin{center}
...
In a \gls{WSSUS} channel model the correlation function is time invariant and that the different scatters and their delays are uncorrelated. This means that the WSSUS channel model only need one parameter per domain. To specify space shifts that the shift in space is unchanged and gives uncorrelated Doppler shifts. Together with the unchanged shifts in frequency. This will simplify \autoref{AutocorrDouble} so it's only dependant two variables. WSSUS model gives a realistic way to describe short term fading of the channel \citep{WSSUSbook}.
...
The amount of time it requires to gather a samples is very important when working with a high number of samples. If one measurement requires 1 second to do for a total of $10^7$ seconds or 116 days of continues measurements. This of course is not practical to do. So it's obvious that a a automatic systems where you take several samples in parallel. If we use the same example as in \autoref{howtomeasureUS} and say the samples are in one dimension with a velocity of $1m/s$. The antenna array is spaced in another dimension so samples don't overlap.The time it would take to do a $100MHz$ sweep is dependant on the $IF_{BW}$ and dynamic range requirements discussed later, but  the time budget assumes at every $\frac{\lambda}{2} = 0.05m$ the setup can sweep the whole BW and get the 30 samples or $N_f$.So first lets calculate the amount of samples we can do per second $N_s$:
\begin{equation}
