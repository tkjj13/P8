...
\end{equation}
If the units of $t$ is seconds,then $f$ is in cycles per second(Hz). 
If the time and space domain are separated  $h(\Delta d)$ $\Delta d$ is in meters and $H$ is the function of spatial frequency measured in cycles per meter or Doppler. \citep{FTandCORR}

Wireless channels autocorrelation $R$ is used to characterize how a random channel will change in time. So it is only dependant on one parameter, the delay $\tau$. Autocorrelations can be found for the temporal(time,$t$), frequency ($f$) and spatial(space,$\Delta d$) domains inherent to wireless random channels. The Power Spectral density $H(t)$, $H(f)$ and $H(\Delta d)$ is a measure of the average spectral power in a random channel. The autocorrelation is related to the power density spectrum by another Fourier transform.  
\begin{equation}
S [cylces \ per \ meter/second]\Leftrightarrow R [meter/second]
\end{equation}
where S is the Power spectra density and R is the autocorrelation.
...
$h(\Delta d,\tau)$\\
\scalebox{0.5}{
${\mathcal{F}\uparrow\downarrow \mathcal{F}^{-1}}$}\\
