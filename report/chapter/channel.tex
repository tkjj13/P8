...
There are several way to get more samples, we have been trough the frequency and space separation to get uncorrelated samples. But a important part is also to a antenna array that will multiply our number of samples taken at each time interval on the reviver and transmitter. So a $5\cdot 5$ antenna array would gives us $5^2 = 25$ uncorrelated channels. If the area of the measurement is not big enough to take all the measurements different rooms could be used. To do this you need to normalize for the mean gain the system sees. If these to ideas are combined in a transmitter and multiple reviver scenario. If we take $5*5$ antenna arrays and place them in 5 different rooms a $5*5*5=125$ uncorrelated samples would be obtained.

WWWWW MAKE A TABLE ABOUT DELAY SPREAD AND ENVIRONMENT Comparison Coherence BW wwww
