...
\label{ch:MassOver}
The focus of this chapter is to provide an overview over the concept and design of the \gls{MDE}. From \autoref{ch:NB-IoT} it is known that \todo{Insert knowledge fact about massive.} and a known concept is known from the project \todo{Insert charly/mathias ref}.
...
The challenges with emulating a massive amount of devices, is that a single device is complex, as seen through \autoref{ch:NB-IoT}, and not easy to emulate, as all the aspect of devices functionalities also is wanted in the emulator, to perform test with in the future. So to emulate a massive amount of device, there will be needed a lot CPU power and memory to process all data for the individual devices and containing the devices them self. Another challenge is that the signals have to be combined for the different devices and send to the eNB and the signal from the eNB have to be split up to all the devices and this have to be handle en quickly as the system is a real time system.

As seen from \todo{Insert tabel ref from ch 2} is there many version of emulators for a single NB-IoT device, but as the protocol still is in development, is these emulator also in development stage only and they only emulate a single device. But as a foundation for the project will the SRS NB-IoT emulator be use as baseline, as it designing the emulator from scratch will \todo{Find a good formulation here}. The SRS emulator is chosen, as it was accessible to get with the source code, as there have to be some restructure in the code, and it is based on the LTE emulator from SRS, which was used in \todo{CM ref}, were the base concept in this project will be taken from.

The SRS NB-IoT emulator is a bigger c++ code project structured up into classes. As it is a third party\todo{Right term?} code and was not intended to emulate multiple devices, there have to be made a higher amount of changes, to make the emulator emulate a massive amount of devices.
