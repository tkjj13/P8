...
%Here should be an introduction to what we will test (the emulator and/or the protocol). 
...
The idle mode power consumption involves the elements $P_{eDRX}$ and $P_{PSM}$. To measure those, the device is released from the cell after the desired idle mode is enabled. The procedure used to measure the idle modes can be found in \appref{app:test_procedures}. The raw data from each of the idle mode measurements can be seen on \autoref{fig:DRX}, \autoref{fig:PSM} and \autoref{fig:eDRX1}.
